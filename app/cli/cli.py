...
async def main_cli():
    console.print(
        Panel(
            "[bold green]Bem-vindo ao Assistente de Automóveis Virtual![/bold green]",
    ...
    while True:
        ...
        if user_input.lower() == "sair":
            console.print(
                Panel("[bold yellow]Até logo![/bold yellow]", border_style="yellow")
            )
            break

        with Live(
            Text("O agente está pensando...", style="italic blue"),
            console=console,
            screen=False,
        ...
        console.print(
            Panel(
                Text(agent_response, style="green"),
                title="[bold green]Agente Automóveis[/bold green]",
            )
        )
        console.print("")
