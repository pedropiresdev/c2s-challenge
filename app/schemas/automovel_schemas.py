...
class AutomovelUpdate(AutomovelBase):
    marca: Optional[str] = Field(
        None, example="Honda", description="Marca do automóvel."
    )
    modelo: Optional[str] = Field(
        None, example="Civic", description="Modelo específico do automóvel."
    )
    ...
    cor: Optional[str] = Field(
        None, example="Branco", description="Cor predominante do automóvel."
    )
    tipo_combustivel: Optional[TipoCombustivel] = Field(
        None,
        example=TipoCombustivel.GASOLINA,
        description="Tipo de combustível que o automóvel utiliza.",
    )
    quilometragem: Optional[float] = Field(
        None, ge=0, example=60000.0, description="Quilometragem atual do automóvel."
    )
    numero_portas: Optional[int] = Field(
        None, ge=2, le=5, example=4, description="Número de portas do automóvel."
    )
    placa: Optional[str] = Field(
        None,
        pattern=r"^[A-Z]{3}[ -]?\d[A-Z\d]?\d{2}$",
        example="XYZ9A87",
        description="Placa do automóvel (formato Mercosul ou antigo).",
    )
    chassi: Optional[str] = Field(
        None,
        pattern=r"^[0-9A-Z]{17}$",
        example="9BWZZZ5X0JP000002",
        description="Número de chassi do automóvel (17 caracteres alfanuméricos).",
    )
