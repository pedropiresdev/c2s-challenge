...
from app.schemas.automovel_schemas import AutomovelCreate, AutomovelUpdate, TipoCombustivel
...
async def test_get_all_automoveis_no_filters(automovel_crud: AutomovelCRUD, sample_automovel_data: AutomovelCreate):
    ...
    await automovel_crud.create_automovel(
        AutomovelCreate(
            ...
            tipo_combustivel=TipoCombustivel.FLEX, quilometragem=30000.0,
            numero_portas=4, placa="XYZ9A87", chassi="1234567890ABCDEF1", codigo_fipe="001234-5"
        )
    )
...
async def test_get_all_automoveis_with_filters(automovel_crud: AutomovelCRUD, sample_automovel_data: AutomovelCreate):
    ...
    await automovel_crud.create_automovel(
        AutomovelCreate(
            marca="Chevrolet", modelo="Cruze", ano=2021, cor="Preto",
            tipo_combustivel=TipoCombustivel.GASOLINA, quilometragem=40000.0,
            numero_portas=4, placa="DEF1G23", chassi="ABCDEF12345678901", codigo_fipe="009876-5"
        )
    )
...
async def test_update_automovel(automovel_crud: AutomovelCRUD, sample_automovel_data: AutomovelCreate):
    ...
    updated_automovel = await automovel_crud.update_automovel(created_automovel.id, update_data)
